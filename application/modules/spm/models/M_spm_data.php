...
class M_spm_data extends CI_Model{
   ...
   function form_urusan(){
    ...
    function get_list_total($where,$where_urusan,$where_sasaran,$like){
    ...
    function get_list_data($where,$where_urusan,$where_sasaran,$like,$limit,$offset) {
        $this->db->select('a.URUSAN_ID, a.SASARAN, b.ID, c.INDIKATOR, c.`2010`, c.`2011`, c.`2012`, c.`2013`, c.`2014`, c.`2015`, c.`2016`, c.`2017`, c.`2018`, c.`2019`, c.`2020`, c.`2021`');
        $this->db->from('tx_spm_sasaran AS a');
        $this->db->join('tx_spm AS b', 'a.ID = b.SASARAN_ID', 'LEFT');
        $this->db->join('v_data_dasar AS c', 'b.INDIKATOR_ID = c.ID_INDIKATOR', 'INNER');
        if($where) {
