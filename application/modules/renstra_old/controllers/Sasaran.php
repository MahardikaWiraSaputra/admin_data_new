...
class Sasaran extends CI_Controller {
    ...
    public function list(){
        ...
        $data['total_items'] = $this->m_renstra_sasaran->get_list_total($param,$like)->row('count');
        $data['list_tujuan'] = $this->m_renstra_sasaran->get_list_tujuan($where, $limit, $offset);

        foreach($data['list_tujuan'] as $key =>$val){
            $sasaran = $this->m_renstra_sasaran->get_list_data($val['ID'],$like);
            if($sasaran){
