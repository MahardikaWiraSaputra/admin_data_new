...
class Kegiatan extends CI_Controller {
    ...
    public function index(){
        if (!$this->ion_auth->in_group(array(1,2)))
        {
            ...
            $data['filter_skpd'] = $this->m_renstra_kegiatan->filter_skpd($id);
            $data['filter_program'] = $this->m_renstra_kegiatan->filter_program($id);
        }
        else
        {
            $data['filter_skpd'] = $this->m_renstra_kegiatan->filter_skpd();
            $data['filter_program'] = $this->m_renstra_kegiatan->filter_program();
        }
    ...
    public function daftar(){
        ...
        $data['total_items'] = $this->m_renstra_kegiatan->get_list_total($param, $where_program, $like)->row('count');
        $data['list_kegiatan'] = $this->m_renstra_kegiatan->get_list_data($param, $where_program, $like, $limit, $offset);

        foreach($data['list_kegiatan'] as $key =>$val){
            $indikator = $this->m_renstra_kegiatan->get_list_indikator($val['ID'],$like);
            if($indikator){
    ...
    public function list_indikator($id){
        $data['list_indikator'] = $this->m_renstra_kegiatan->get_indikator_by_id($id);
        $this->load->view('renstra/kegiatan/v_indikator_list', $data);
    }
    ...
    public function remove_indikator($id){
        if($id) {         
            $query = $this->m_renstra_kegiatan->remove_indikator($id);
            if ($query) {
                $output['success'] = true;
                $output['message'] = 'DATA BERHASIL DIHAPUS';
            }
