...
class M_rpjmd_sasaran extends CI_Model{
    ...
    function detail($id){
        $this->db->select('a.URUSAN_ID, e.KODE_URUSAN, e.URUSAN, a.TUJUAN_ID, b.TUJUAN, b.MISI_ID, c.MISI, c.VISI_ID, d.VISI, a.ID, a.SASARAN');
        $this->db->from('tx_rpjmd_sasaran AS a');
    ...
    function get_indikator_by_id($id){
        $this->db->select('a.ID, a.SASARAN_ID, a.INDIKATOR_ID, b.SASARAN, c.INDIKATOR, c.SATUAN, c.`2010`, c.`2011`, c.`2012`, c.`2013`, c.`2014`, c.`2015`, c.`2016`, c.`2017`, c.`2018`, c.`2019`, c.`2020`, c.`2021`');
        $this->db->from('tx_rpjmd_sasaran_indikator AS a');
        $this->db->join('tx_rpjmd_sasaran AS b', 'a.SASARAN_ID = b.ID', 'LEFT');
