					...
					<tr>
						...
						<td class="text-center">
							<?php if($row[$group->name] == '1') { ; ?>
								<span class="feather-sub text-success"><i data-feather="check-circle"></i></span>
							<?php } else { ?>
								<span class="feather-sub text-warning"><i data-feather="x-circle"></i></span> 
							<?php } ?></td>
						<?php endforeach; ?>
						<td>
							<div class="btn-group">
								<button type="button" class="btn btn-info btn-xs btn-info" onclick="edit('<?php echo $row['perm_id']; ?>')">Edit</button>
								<button type="button" class="btn btn-info btn-xs btn-danger" onclick="hapus('<?php echo $row['perm_id']; ?>')">Del</button>
							</div>
						</td>
					</tr>
					<?php endforeach;?>
...
<script>
	...
	$('#show_paginator').bootpag({
	    ...
	    function edit(id){
	        ajax_modal('master/group_permissions/edit/'+id);
	    }
	    ...
	    function hapus(id) {
	        swal({
	            ...
	            if (result.value) {
	                $.ajax({
	                    url  : "<?php echo base_url()?>master/group_permissions/delete/"+id,
	                    dataType: "JSON",
	                    type: "POST",
